...
void debug_f4(const char *template,
  ...
  char buf[1024];
  int count = snprintf(buf, 1000, template,
...
void debug_f3(const char *template,
  ...
  if(!debug_cfg.enable_debug_msgs) return;
  char buf[1024];
  int count = snprintf(buf, 1000, template, value0, value1, value2);
  buf[count] = 0;
  debug_f0(buf);
}
...
void debug_f2(const char *template,
  ...
  if(!debug_cfg.enable_debug_msgs) return;
  char buf[1024];
  int count = snprintf(buf, 1000, template, value0, value1);
  buf[count] = 0;
  debug_f0(buf);
}
...
void debug_f1(const char *template,
  ...
  if(!debug_cfg.enable_debug_msgs) return;
  char buf[1024];
  int count = snprintf(buf, 1000, template, value);
  buf[count] = 0;
  debug_f0(buf);
}
...
void debug_f0(const char *template) {
  ...
  debug_msg.dptr = malloc(1024);
  int count = snprintf((char *)debug_msg.dptr, 1000, template);
  ((char *)debug_msg.dptr)[count] = 0;
